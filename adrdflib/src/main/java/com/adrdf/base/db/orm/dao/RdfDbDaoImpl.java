package com.adrdf.base.db.orm.dao;
...
import com.adrdf.base.db.base.RdfBaseDBDao;
import com.adrdf.base.db.orm.annotation.ActionType;
import com.adrdf.base.db.orm.annotation.Column;
import com.adrdf.base.db.orm.annotation.Id;
import com.adrdf.base.db.orm.annotation.Relations;
import com.adrdf.base.db.orm.annotation.RelationsType;
 ...
 *
 * Name：RdfDbDaoImpl
 * Describe：数据库表操作类接口实现类
 * Date：2017-03-17 11:36:23
 * Author: CapRobin@yeah.net
 *
 */
public class RdfDbDaoImpl<T> extends RdfBaseDBDao implements RdfDbDao<T> {
	 ...
	 * @param clazz 映射对象实体
	 */
	public RdfDbDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		// 加载所有字段
		this.allFields = RdfTableCreater.joinFields(this.clazz.getDeclaredFields(),
		...
		RdfLogUtil.d(RdfDbDaoImpl.class, "clazz:" + this.clazz + " tableName:" + this.tableName
	 ...
	 * @param dbHelper 数据库操作实现类
	 */
	public RdfDbDaoImpl(SQLiteOpenHelper dbHelper) {
		this(dbHelper, null);
	}
	...
	public T queryOne(int id) {
		synchronized (lock) {
			...
			RdfLogUtil.d(RdfDbDaoImpl.class, "[queryOne]: select * from " + this.tableName + " where "
					+ this.idColumn + " = '" + id + "'");
	...
	public List<T> rawQuery(String sql, String[] selectionArgs,Class<T> clazz) {
		...
		try {
			lock.lock();
			checkDBOpened();
			RdfLogUtil.d(RdfDbDaoImpl.class, "[rawQuery]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			getListFromCursor(clazz,list, cursor);
		} catch (Exception e) {
			RdfLogUtil.e(RdfDbDaoImpl.class, "[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			RdfLogUtil.d(RdfDbDaoImpl.class, "[isExist]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			if (cursor.getCount() > 0) {
				return true;
			}
		} catch (Exception e) {
			RdfLogUtil.e(RdfDbDaoImpl.class, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public List<T> queryList(String[] columns, String selection,
			...
			try {
				lock.lock();
				checkDBOpened();
				RdfLogUtil.d(RdfDbDaoImpl.class, "[queryList] from "+this.tableName+" where "+selection+"("+selectionArgs+")"+" group by "+groupBy+" having "+having+" order by "+orderBy+" limit "+limit);
					...
					for(T entity:list){
                        ...
                        if(RelationsType.one2one.equals(type)){
                            ...
                            if(listEntityClazz==null){
                                RdfLogUtil.e(RdfDbDaoImpl.class, "对象模型需要设置List的泛型");
                                return null;
                            }
				...
				RdfLogUtil.e(RdfDbDaoImpl.class, "[queryList] from DB Exception");
				e.printStackTrace();
			} finally {
				closeCursor(cursor);
				lock.unlock();
	...
	private void getListFromCursor(Class<?> clazz,List<T> list, Cursor cursor)
		...
		while (cursor.moveToNext()) {
			...
			// 加载所有字段
			List<Field> allFields = RdfTableCreater.joinFields(entity.getClass().getDeclaredFields(),
	...
	public long insert(T entity) {
			...
			try {
				...
				RdfLogUtil.d(RdfDbDaoImpl.class, "[insert]: insert into " + this.tableName + " " + sql);
				...
				for (Field relationsField : allFields) {
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							RdfLogUtil.d(RdfDbDaoImpl.class, "[insert]: insert into " + relationsTableName + " " + sql);
							try {
				...
				RdfLogUtil.d(RdfDbDaoImpl.class, "[insert] into DB Exception.");
				e.printStackTrace();
			}finally {
				lock.unlock();
			}
	...
	public long[] insertList(List<T> entityList) {
			...
			try {
				...
				for(int i=0;i<entityList.size();i++){
					...
					RdfLogUtil.d(RdfDbDaoImpl.class, "[insertList]: insert into " + this.tableName + " " + sql);
					...
					if(RelationsType.one2one.equals(type)){
						...
						if(relationsEntity != null){
							...
							RdfLogUtil.d(RdfDbDaoImpl.class, "[insertList]: insert into " + relationsTableName + " " + sql);
							try {
								db.insert(relationsTableName, null, relationsCv);
							} catch (Exception e) {
							}
				...
				RdfLogUtil.d(RdfDbDaoImpl.class, "[insertList] into DB Exception.");
				e.printStackTrace();
			} finally {
				lock.unlock();
			}
	...
	public int delete(int id) {
		...
		try {
			...
			RdfLogUtil.d(RdfDbDaoImpl.class, "[delete]: delelte from " + this.tableName + " where "
			...
			for (Field relationsField : allFields) {
				...
				RdfLogUtil.d(RdfDbDaoImpl.class, "[delete]: delete from " + relationsTableName + "where "+foreignKey+" = "+String.valueOf(id)+";");
	...
	public int delete(String whereClause, String[] whereArgs) {
		...
		try {
			...
			if(!RdfStrUtil.isEmpty(sql)){
				sql +=" where ";
			}
			RdfLogUtil.d(RdfDbDaoImpl.class, "[delete]: delete from " + this.tableName + sql);
	...
	public int deleteAll() {
		...
		try {
			lock.lock();
			checkDBOpened();
			RdfLogUtil.d(RdfDbDaoImpl.class, "[delete]: delete from " + this.tableName );
			...
			for (Field relationsField : allFields) {
				...
				RdfLogUtil.d(RdfDbDaoImpl.class, "[delete]: delete from " + relationsTableName + ";");
			...
			RdfLogUtil.d(RdfDbDaoImpl.class, "[delete] DB Exception.");
			e.printStackTrace();
		}finally{
			lock.unlock();
		}
	...
	public int update(T entity) {
		...
		try {
			...
			RdfLogUtil.d(RdfDbDaoImpl.class, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(id)));

			String[] whereValue = { Integer.toString(id) };
			rows = db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			RdfLogUtil.d(RdfDbDaoImpl.class, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
	...
	public List<Map<String, String>> queryMapList(String sql,String[] selectionArgs) {
		...
		try {
			lock.lock();
			checkDBOpened();
			RdfLogUtil.d(RdfDbDaoImpl.class, "[queryMapList]: " + getLogSql(sql, selectionArgs));
			cursor = db.rawQuery(sql, selectionArgs);
			while (cursor.moveToNext()) {
			...
			RdfLogUtil.e(RdfDbDaoImpl.class, "[queryMapList] from DB exception");
			e.printStackTrace();
		} finally {
			closeCursor(cursor);
			lock.unlock();
	...
	public int queryCount(String sql, String[] selectionArgs) {
       ...
       try{
    	   lock.lock();
    	   checkDBOpened();
    	   RdfLogUtil.d(RdfDbDaoImpl.class, "[queryCount]: " + getLogSql(sql, selectionArgs));
           cursor = db.query(this.tableName, null, sql, selectionArgs, null, null,null);
           if(cursor != null){
        	   count = cursor.getCount();
           }
       }catch (Exception e){
    	   RdfLogUtil.e(RdfDbDaoImpl.class, "[queryCount] from DB exception");
           e.printStackTrace();
       }finally{
    	   closeCursor(cursor);
    	   lock.unlock();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		try {
			lock.lock();
			checkDBOpened();
			RdfLogUtil.d(RdfDbDaoImpl.class, "[execSql]: " + getLogSql(sql, selectionArgs));
			...
			RdfLogUtil.e(RdfDbDaoImpl.class, "[execSql] DB exception.");
			e.printStackTrace();
		} finally {
			lock.unlock();
		}
