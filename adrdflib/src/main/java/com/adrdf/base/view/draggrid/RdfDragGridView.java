package com.adrdf.base.view.draggrid;
...
public class RdfDragGridView extends GridView implements GridView.OnScrollListener{
    ...
    private OnItemLongClickListener onLongClickListener = new OnItemLongClickListener(){
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            //设置被长按item不显示
            ((RdfDragGridViewAdapter)getAdapter()).hideView(position);
            return true;
        }
    };

    public RdfDragGridView(Context context) {
        super(context);
        initView();
    }

    public RdfDragGridView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public RdfDragGridView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        //dragImageView处于被拖动时，更新dragImageView位置
            ...
            if((currDraggedPosition != AdapterView.INVALID_POSITION) && (currDraggedPosition != preDraggedOverPositon)) {
                ((RdfDragGridViewAdapter)getAdapter()).swapView(preDraggedOverPositon, currDraggedPosition);
                preDraggedOverPositon = currDraggedPosition;
            }
        }
        //释放dragImageView
        else if((ev.getAction() == MotionEvent.ACTION_UP) && (isViewOnDrag == true)) {
            ((RdfDragGridViewAdapter)getAdapter()).showHideView();
            if((int)dragImageView.getTag() == DRAG_IMG_SHOW) {
                windowManager.removeView(dragImageView);
                dragImageView.setTag(DRAG_IMG_NOT_SHOW);
            }
