package com.adrdf.base.view.photo.region;
...
import com.adrdf.base.view.photo.RdfPhotoImageView;
...
public class TileDrawable extends Drawable {
    ...
    private void initTiles(Point container) {
        if (mTiles == null) {
            ...
            RdfPhotoImageView.MathUtils.calculateScaledRectInContainer(new RectF(0, 0, sWidth, sHeight), iWidth, iHeight, ImageView.ScaleType.FIT_CENTER, scaledRect);
    ...
    private Handler mRequestCurrentTilesDelayHandler = new Handler(new Handler.Callback() {
        ...
        public boolean handleMessage(Message msg) {
            if (getCallback() != null && getCallback() instanceof RdfPhotoImageView) {
                requestCurrentTiles((RdfPhotoImageView) getCallback());
    ...
    private void requestCurrentTiles(RdfPhotoImageView pinchImageView) {
        if (mTiles != null && pinchImageView.getWidth() > 0 && pinchImageView.getHeight() > 0) {
            ...
            float scale = RdfPhotoImageView.MathUtils.getMatrixScale(matrix)[0];
            ...
            for (int i = mTiles.length - 1; i >= 0; i--) {
                for (Tile tile : mTiles[i]) {
                    if (tile.mStatus == Tile.STATUS_LOADED && tile.mBitmap != null) {
                        ...
                        RdfPhotoImageView.MathUtils.calculateRectTranslateMatrix(new RectF(0, 0, tile.mBitmap.getWidth(), tile.mBitmap.getHeight()), new RectF(tile.mSampleRect), matrix);
