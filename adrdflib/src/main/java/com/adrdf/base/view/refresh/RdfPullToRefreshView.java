 ...
 * Copyright © CapRobin
 *
 * Name：RdfPullToRefreshView
 * Describe：下拉刷新和加载更多的View
 * Date：2017-06-14 11:22:40
 * Author: CapRobin@yeah.net
 *
 */
public class RdfPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private RdfListViewBaseHeader mHeaderView;
    private RdfListViewHeader defaultViewHeader;

    /** footer view. */
	private RdfListViewFooter mFooterView;
	 ...
	 */
	public RdfPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param context the context
	 */
	public RdfPullToRefreshView(Context context) {
		super(context);
		init(context);
	}
	...
	private void init(Context context) {
        ...
        defaultViewHeader = new RdfListViewHeader(mContext);
		setHeaderView(defaultViewHeader);
	}

    public RdfListViewHeader getDefaultHeaderView() {
	 ...
	 * 增加头布局.
	 */
	public void setHeaderView(RdfListViewBaseHeader headerView) {

        this.mHeaderView = headerView;
		//获取View的高度
		RdfViewUtil.measureView(this.mHeaderView);
	...
	private void addFooterView() {
        ...
        mFooterView = new RdfListViewFooter(mContext);
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != RdfListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(RdfListViewHeader.STATE_READY);
		
		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(RdfListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != RdfListViewFooter.STATE_LOADING) {
			mFooterView.setState(RdfListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(RdfListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(RdfListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(RdfListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(RdfListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				mFooterView.setState(RdfListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(RdfListViewFooter.STATE_READY);
		}
     ...
     */
    public RdfListViewBaseHeader getHeaderView() {
        return mHeaderView;
    }
     ...
     */
    public RdfListViewFooter getFooterView() {
        return mFooterView;
    }
