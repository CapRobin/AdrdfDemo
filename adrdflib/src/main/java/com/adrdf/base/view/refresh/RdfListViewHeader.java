 ...
 * Copyright © CapRobin
 *
 * Name：RdfListViewHeader
 * Describe：下拉刷新的Header View类
 * Date：2017-06-27 20:22:20
 * Author: CapRobin@yeah.net
 *
 */
public class RdfListViewHeader extends RdfListViewBaseHeader {
	 ...
	 */
	public RdfListViewHeader(Context context) {
		super(context);
		initView();
	}
	...
	private void initView() {
		...
		RdfViewUtil.setPadding(headerView, 0, 10, 0, 10);
		...
		//箭头图片
		arrowImageView.setImageResource(com.adrdf.library.R.drawable.arrow);
		...
		layoutParamsWW.width = RdfViewUtil.scaleValue(context, 50);
		layoutParamsWW.height = RdfViewUtil.scaleValue(context, 50);
		...
		RdfViewUtil.setPadding(headTextLayout,0, 0, 0, 0);
		...
		RdfViewUtil.setTextSize(tipsTextview,30);
		RdfViewUtil.setTextSize(headerTimeView,27);
		...
		layoutParamsWW3.rightMargin = RdfViewUtil.scaleValue(context, 10);
		...
		switch(state){
			case STATE_NORMAL:
				...
				if(lastRefreshTime==null){
					lastRefreshTime = RdfDateUtil.getCurrentDate(RdfDateUtil.dateFormatHMS);
