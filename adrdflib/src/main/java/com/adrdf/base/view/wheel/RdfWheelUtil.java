 ...
 * Name：RdfWheelUtil
 * Describe：轮子工具类
 * Date：2017-06-18 18:29:53
 * Author: CapRobin@yeah.net
 *
 */

public class RdfWheelUtil {
	 ...
	 * @param maxYear     结束的年
	 */
	public static void initWheelPickerYMD(final RdfWheelView mWheelViewY, final RdfWheelView mWheelViewM, final RdfWheelView mWheelViewD,
		...
		//设置"年"的显示数据
		mWheelViewY.setAdapter(new RdfNumericWheelAdapter(minYear, maxYear));
		...
		// 月
		mWheelViewM.setAdapter(new RdfNumericWheelAdapter(1, 12));
		...
		if (list_big.contains(String.valueOf(month))) {
			mWheelViewD.setAdapter(new RdfNumericWheelAdapter(1, 31));
		} else if (list_little.contains(String.valueOf(month))) {
			mWheelViewD.setAdapter(new RdfNumericWheelAdapter(1, 30));
		} else {
			// 闰年
			if (RdfDateUtil.isLeapYear(year)){
				mWheelViewD.setAdapter(new RdfNumericWheelAdapter(1, 29));
			}else{
				mWheelViewD.setAdapter(new RdfNumericWheelAdapter(1, 28));
			}
		}
		...
		// 添加"年"监听
		RdfWheelView.AbOnWheelChangedListener wheelListener_year = new RdfWheelView.AbOnWheelChangedListener() {

			public void onChanged(RdfWheelView wheel, int oldValue, int newValue) {
				...
				if (list_big.contains(String.valueOf(mWheelViewM.getCurrentItem() + 1))) {
					...
					if (RdfDateUtil.isLeapYear(year_num))
						mWheelViewD.setAdapter(new RdfNumericWheelAdapter(1, 29));
					else
						mWheelViewD.setAdapter(new RdfNumericWheelAdapter(1, 28));
				}
		...
		// 添加"月"监听
		RdfWheelView.AbOnWheelChangedListener wheelListener_month = new RdfWheelView.AbOnWheelChangedListener() {
	 ...
	 */
	public static void initWheelPickerMDHM(final RdfWheelView mWheelViewMD, final RdfWheelView mWheelViewHH, final RdfWheelView mWheelViewMM){
		Calendar calendar = Calendar.getInstance();
		int year = calendar.get(Calendar.YEAR);
		int month = calendar.get(Calendar.MONTH)+1;
		int day = calendar.get(Calendar.DATE);
	...
	public static void initWheelPickerMDHM(final RdfWheelView mWheelViewMD, final RdfWheelView mWheelViewHH, final RdfWheelView mWheelViewMM,
		...
		for(int i=1;i<13;i++){
			if(list_big.contains(String.valueOf(i))){
				...
				if(i==2){
					if(RdfDateUtil.isLeapYear(defaultYear)){
						for(int j=1;j<28;j++){
							textDMList.add(i+"月"+" "+j+"日");
							MDHMList.add(defaultYear+"-"+i+"-"+j);
						}
		...
		// 月日
		mWheelViewMD.setAdapter(new RdfStringWheelAdapter(textDMList));
		...
		// 时
		mWheelViewHH.setAdapter(new RdfNumericWheelAdapter(0, 23));
		...
		// 分
		mWheelViewMM.setAdapter(new RdfNumericWheelAdapter(0, 59));
	 ...
	 * @param mWheelViewMM  选择分的轮子
	 */
	public static void initWheelPickerHM(final RdfWheelView mWheelViewHH, final RdfWheelView mWheelViewMM) {
		Calendar calendar = Calendar.getInstance();
		int hour = calendar.get(Calendar.HOUR_OF_DAY);
		int minute = calendar.get(Calendar.MINUTE);
