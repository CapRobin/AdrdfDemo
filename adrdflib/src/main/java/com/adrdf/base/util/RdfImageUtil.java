 ...
 * Name：RdfImageUtil
 * Describe：图片工具类
 * Date：2017-01-06 17:39:47
 * Author: CapRobin@yeah.net
 *
 */

public class RdfImageUtil {
	...
	public static Bitmap getBitmap(String url,int desiredWidth, int desiredHeight) {
		...
		try {
			...
			e.printStackTrace();
			RdfLogUtil.d(RdfImageUtil.class, "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	public static Bitmap getBitmap(InputStream inputStream,int desiredWidth, int desiredHeight) {
        ...
        try {
            ...
            if(desiredWidth <= 0 || desiredHeight <= 0){
                ...
                byte [] data =  RdfStreamUtil.stream2Bytes(inputStream);
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			RdfLogUtil.e(RdfImageUtil.class, "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			RdfLogUtil.e(RdfImageUtil.class, "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			RdfLogUtil.e(RdfImageUtil.class, "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
	...
	public static byte[] view2Bytes(View view,
		...
		byte[] b = null;
		try {
			Bitmap bitmap = RdfImageUtil.view2Bitmap(view);
			b = RdfImageUtil.bitmap2Bytes(bitmap, compressFormat, true);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return b;
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					RdfLogUtil.d(RdfImageUtil.class,
	...
	public static void releaseBitmapList(List<Bitmap> bitmaps) {
		if (bitmaps != null) {
			try {
				for (Bitmap bitmap : bitmaps) {
					if (bitmap != null && !bitmap.isRecycled()) {
						RdfLogUtil.d(RdfImageUtil.class,"Bitmap释放" + bitmap.toString());
						bitmap.recycle();
					}
				}
			} catch (Exception e) {
