...
import com.adrdf.base.model.RdfAppProcessInfo;
import com.adrdf.base.model.RdfCpuInfo;
import com.adrdf.base.model.RdfProcessInfo;
import com.adrdf.base.model.RdfPsRow;
 ...
 * Name：RdfAppUtil
 * Describe：APP工具类
 * Date：2017-08-27 11:02:18
 * Author: CapRobin@yeah.net
 *
 */
public class RdfAppUtil {
     ...
     * @return
     */
    public static List<RdfAppProcessInfo> getRunningAppProcesses(Context context) {
    	ActivityManager activityManager = null;
    	List<RdfAppProcessInfo> list = null;
    	PackageManager packageManager = null;
	    try {
	    	activityManager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
	    	packageManager = context.getApplicationContext().getPackageManager();
	    	list = new ArrayList<RdfAppProcessInfo>();
	    	//所有运行的进程
	    	List<RunningAppProcessInfo> appProcessList = activityManager.getRunningAppProcesses();
	    	ApplicationInfo appInfo = null;
	    	RdfAppProcessInfo abAppProcessInfo = null;
	    	...
	    	for(RunningAppProcessInfo appProcessInfo:appProcessList){
	    		abAppProcessInfo = new RdfAppProcessInfo(appProcessInfo.processName,appProcessInfo.pid,appProcessInfo.uid);
	    		...
	    		RdfProcessInfo processInfo = getMemInfo(appProcessInfo.processName);
	 ...
	 * 执行PS.
	 */
    public static List<RdfPsRow> ps() {
    	List<RdfPsRow> psRowlist = new ArrayList<RdfPsRow>();
        String ps = runScript("ps");
        String[] lines = ps.split("\n");
        psRowlist = new ArrayList<RdfPsRow>();
        for (String line : lines) {
        	RdfPsRow row = new RdfPsRow(line);
     ...
     * @return
     */
    public static RdfPsRow getPsRow(String processName) {
    	List<RdfPsRow> psRowlist = ps();
        for (RdfPsRow row : psRowlist) {
     ...
     * @return
     */
    public static RdfProcessInfo getMemInfo(String processName) {
    	RdfProcessInfo process = new RdfProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
        ...
        if(process.memory == 0){
        	RdfLogUtil.d(RdfAppUtil.class, "##"+processName+",top -n 1未找到");
        }
        return process;
    }
     ...
     * @return
     */
    public static RdfProcessInfo getMemInfo(int pid) {
    	RdfProcessInfo process = new RdfProcessInfo();
    	if(mProcessList == null){
    		mProcessList = getProcessRunningInfo();
    	}
	...
	public static String runCommand(String[] command, String workdirectory){
		String result = "";
		RdfLogUtil.d(RdfAppUtil.class, "#"+command);
		try {
	 ...
	 * @return
	 */
	public static RdfCpuInfo getCPUInfo() {
		RdfCpuInfo CPUInfo = null;
		try {            
			String result = runCommandTopN1();  
	 ...
	 * @return
	 */
	public static RdfCpuInfo parseCPUInfo(String info) {
		 RdfCpuInfo CPUInfo = new RdfCpuInfo();
		 String tempString = "";
		 String[] rows = null;
		 String[] columns = null;
		 rows = info.split("[\n]+");
	...
	public static long getTotalMemory(Context context){  
        ...
        try{  
            ...
            for(String str:strs){  
                RdfLogUtil.d(RdfAppUtil.class,str+"\t");
            }  
            //获得系统总内存，单位KB  
	...
	public static boolean requestSDCardPermission(Context context) {
		...
		if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)!= PackageManager.PERMISSION_GRANTED) {
			...
			if (ActivityCompat.shouldShowRequestPermissionRationale((Activity)context,
				...
				RdfToastUtil.showToast(context,"SDCard访问权限被拒绝，请在设置-应用程序打开!");
				return false;
			} else {
				//进行权限请求
	...
	public static void clearAllCache(Context context){
		try {
			...
			RdfLogUtil.d(RdfDiskCacheImpl.class,"清除缓存成功.");
		}catch(Exception e){
			e.printStackTrace();
			RdfLogUtil.d(RdfDiskCacheImpl.class,"清除缓存失败.");
		}
	...
	public static void clearAllDownload(Context context){
		try {
			...
			if (!RdfFileUtil.isCanUseSD()) {
				...
				downloadDir = new File(RdfFileUtil.getDownloadRootDir(context));
			}
