...
public class RdfFileUtil {
	 ...
	 public static Bitmap getBitmapFromSD(File file,int type,int desiredWidth, int desiredHeight){
		 ...
		 try {
			 ...
			 if(desiredWidth <= 0 || desiredHeight <= 0){
				 type = RdfImageUtil.ORIGINALIMG;
			 }
			 //文件存在
			 if(type == RdfImageUtil.CUTIMG){
				bitmap = RdfImageUtil.getCutBitmap(file,desiredWidth,desiredHeight);
		 	 }else if(type == RdfImageUtil.SCALEIMG){
		 		bitmap = RdfImageUtil.getScaleBitmap(file,desiredWidth,desiredHeight);
		 	 }else{
		 		bitmap = RdfImageUtil.getBitmap(file);
		 	 }
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Bitmap getBitmapFromURL(String url,int desiredWidth, int desiredHeight){
		Bitmap bitmap = null;
		try {
			bitmap = RdfImageUtil.getBitmap(url,desiredWidth,desiredHeight);
	    } catch (Exception e) {
	    	RdfLogUtil.d(RdfFileUtil.class, "下载图片异常："+e.getMessage());
		}
		return bitmap;
	}
	...
	public static Bitmap getBitmapFromSrc(String src){
		Bitmap bit = null;
		try {
			bit = BitmapFactory.decodeStream(RdfFileUtil.class.getResourceAsStream(src));
	    } catch (Exception e) {
	    	RdfLogUtil.d(RdfFileUtil.class, "获取图片异常："+e.getMessage());
		}
		return bit;
	}
 	 ...
 	 public static String downloadFile(String url,String dirPath){
 		 ...
 		 try {
            ...
            if(!file.exists()){
 			...
 			e.printStackTrace();
 			RdfLogUtil.e(RdfFileUtil.class, "有文件下载出错了,已删除");
	...
	public static int getContentLengthFromUrl(String Url){
		...
		try {
	    	 ...
	    	 e.printStackTrace();
	    	 RdfLogUtil.d(RdfFileUtil.class, "获取长度异常："+e.getMessage());
		}
    ...
    public static String getRealFileNameFromUrl(String url){
        ...
        try {
            ...
            e.printStackTrace();
            RdfLogUtil.e(RdfFileUtil.class, "网络上获取文件名失败");
        }
        return name;
    }
    ...
    public static String getCacheFileNameFromUrl(String url,HttpResponse response){
        ...
        try {
            ...
            if(RdfStrUtil.isEmpty(suffix)){
                suffix = ".ab";
            }
            name = RdfMd5.MD5(url)+suffix;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return name;
	...
	public static String getCacheFileNameFromUrl(String url,HttpURLConnection connection){
		...
		try {
			...
			name = RdfMd5.MD5(url+System.currentTimeMillis())+suffix;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return name;
   ...
   public static void copyAssets2SD(Context context, String assetDir, String outDir) {
		...
		try {
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (filesChild!=null && filesChild.length>0) {
					...
					InputStream in = null;
					if(!RdfStrUtil.isEmpty(assetDir)){
	...
	public static void initFileDir(Context context){
		...
		//默认下载文件根目录. 
		String downloadRootPath = File.separator + RdfAppConfig.DOWNLOAD_ROOT_DIR + File.separator + info.packageName + File.separator;
		
	    //默认下载图片文件目录. 
		String imageDownloadPath = downloadRootPath + RdfAppConfig.DOWNLOAD_IMAGE_DIR + File.separator;
	    
	    //默认下载文件目录.
		String fileDownloadPath = downloadRootPath + RdfAppConfig.DOWNLOAD_FILE_DIR + File.separator;
		
		//默认缓存目录.
		String cacheDownloadPath = downloadRootPath + RdfAppConfig.CACHE_DIR + File.separator;
		
		//默认DB目录.
		String dbDownloadPath = downloadRootPath + RdfAppConfig.DB_DIR + File.separator;
