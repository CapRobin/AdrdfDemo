...
import com.adrdf.base.view.refresh.RdfListViewFooter;
import com.adrdf.base.view.refresh.RdfListViewHeader;
 ...
 * Name：RdfViewUtil
 * Describe：View工具类
 * Date：2016-10-17 11:11:03
 * Author: CapRobin@yeah.net
 *
 */

public class RdfViewUtil {
    ...
    public static float dip2px(Context context, float dipValue) {
        DisplayMetrics mDisplayMetrics = RdfAppUtil.getDisplayMetrics(context);
	...
	public static int scaleValue(Context context, float value) {
		...
		if(mDisplayMetrics.scaledDensity == RdfAppConfig.UI_DENSITY){
			//密度
			if(width > RdfAppConfig.UI_WIDTH){
				value = value*(1.3f - 1.0f/mDisplayMetrics.scaledDensity);
			}else if(width < RdfAppConfig.UI_WIDTH){
				value = value*(1.0f - 1.0f/mDisplayMetrics.scaledDensity);
			}
		}else{
			//密度小屏幕大:缩小比例
			float offset = RdfAppConfig.UI_DENSITY-mDisplayMetrics.scaledDensity;
			if(offset > 0.5f){
				value = value * 0.9f;
			}else{
				value = value * 0.95f;
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		try {
			...
			float scaleWidth = (float) width / RdfAppConfig.UI_WIDTH;
			float scaleHeight = (float) height / RdfAppConfig.UI_HEIGHT;
    ...
    public static void scaleContentView(ViewGroup contentView){
    	RdfViewUtil.scaleView(contentView);
    ...
    public static boolean isNeedScale(View view){
    	if (view instanceof RdfListViewHeader){
    		return false;
        }
    	
    	if (view instanceof RdfListViewFooter){
    		return false;
        }
    	return true;
    }
    ...
    */
    public static void setViewSize(View view,int widthPixels, int heightPixels){
        ...
        if(params == null){
            RdfLogUtil.e(RdfViewUtil.class, "setViewSize出错,如果是代码new出来的View，需要设置一个适合的LayoutParams");
            return;
        }
